...
### 全ての変更は新規ブランチで実施
**develop に直接 commit・push しない。** 全ての変更（機能追加、ドキュメント、テスト等）は以下のフローで実施すること：

```bash
git checkout -b <ブランチ名>        # 新規ブランチを作成（develop から分岐）
# ... 実装 ...
git add .
git commit -m "<メッセージ>"
...
- **develop は保護ブランチ**: 直接 push を禁止し、PR 経由でのみマージを受け付ける
- **PR 作成後は待機**: PR 作成後、マージ・追加 push はユーザーの明示的な指示を待つ

### セルフレビュー必須
実装完了後（コミット前）に、変更コードを確認して明らかな問題を潰しておく。

### UI変更時の動作確認必須
PR作成後、変更差分に UI 関連ファイル（`*.html`, `*.css`, `*.js`）が含まれる場合は、必ず実装が正常に動作するか確認すること。

### 計画実行時の承認フロー（`/goal` + `/plan` 使用時）
`/goal` で目標を設定し `/plan` で計画を立てた場合：
- **実装フェーズ**: 計画に従って実装を進める
- **コミット**: 完了後にコミットまで進める
- **push・PR 作成**: 上記「全ての変更は新規ブランチで実施」に従い、PR を作成
- **マージ**: ユーザーからの明示的な指示を待つ

...
### 計画実行時の承認フロー
`/goal` で目標を設定し `/plan` で計画を立てた場合、勝手に push・PR作成・マージしない。
- **実装フェーズ**: 計画に従って実装を進める
- **コミット**: 完了後にコミットまで進める
- **push 前に確認**: push する前にユーザーに「push して PR 作成してもいいですか？」と確認を取る
- **PR作成**: ユーザーの承認後に push → PR作成（`gh pr create`）を実行
- **マージ**: PR マージはさらにユーザーの指示を待つ
